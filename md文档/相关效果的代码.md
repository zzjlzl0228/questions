...
- 在主页面得index中写好左侧导航得布局

```javascript
<div class="nav-wrapper">
				<ul class="lists">
					<li class="active">
						<a href="link/1/index.html" class="iframe_link"><img src="img/rili.png">每日行业</a>
					</li>
					<li>
						<a href="link/1/index.html" class="iframe_link">每月行业</a>
					</li>
					<li>
						<a href="link/1/index.html" class="iframe_link">每月不重复行业</a>
					</li>
					<li>
						<a href="link/1/index.html" class="iframe_link">员工人数对比</a>
					</li>
					<li>
						<a href="link/1/demo.html" class="iframe_link">工资对比</a>
					</li>
					<li>
						<a href="link/1/demo.html" class="iframe_link">现金账对比</a>
					</li>
					<li>
						<a href="link/1/demo.html" class="iframe_link">项目统计对比</a>
					</li>
					<li>
						<a href="link/1/demo.html" class="iframe_link">每日项目统计对比</a>
					</li>
				</ul>
			</div>
```

- 写好右侧内容部分，在新建个文件夹存放页面，在主页面index.html中写右侧内容部分

```javascript
<div class="main" style="background: #f4f6fa;">
...
```javascript
<div class="news_wait_ten" style="text-align:center;font-size:0.3rem;line-height:0.52rem;">下拉加载更多</div>
// 上面为上拉加载更多
$(function() {
	//ajax加载新数据
	var stop1 = true;
	...
	var stop4 = true;
	var offset_ten = 20;
	var offset_company = 10;
$(window).scroll(function() {
	totalheight = parseFloat(window.innerHeight) + parseFloat($(window).scrollTop());
	if (totalheight == $(document).height()) {
var type = $('.remenContainer').css('display');
var type1 = $('.newsContainer').css('display');
var type2 = $('.hangyeContainer').css('display');
var type3 = $('.storyContainer').css('display');
        // 判断哪个列表容器显示，四个列表容器的type类型值均为1，2，3，4，判断其显示是哪个列表容器，加载哪个数据
	if (type == 'block') {
		type = 1;
		if (stop1 == true) {
								$('.news_wait_ten').text('正在加载');
			stop1 = false;
								$.post("news_ajax.php", {
		type: type,
		offset_ten: offset_ten
},
function(data) {
	if (data) {
		$(".news_wait_ten").before(data);
		$('.news_wait_ten').text('下拉加载更多');
		offset_ten += 20;
		stop1 = true;
	} else {												$('.news_wait_ten').text('没有更多新闻了');
	}
}, "json");
}
} else if (type1 == 'block') {
	...
	if (stop2 == true) {
		...
		function(data) {
			if (data) {
				$(".company_wait_ten").before(data);
		$('.company_wait_ten').text('下拉加载更多');
			offset_ten += 20;
			stop2 = true;
		} else {
											$('.company_wait_ten').text('没有更多新闻了');
										}
									}, "json");;
							}
						} else if (type2 == 'block') {
	...
	offset_ten: offset_ten
									...
									function(data) {
										if (data) {
											$(".hangye_wait_ten").before(data);
											$('.hangye_wait_ten').text('下拉加载更多');
											offset_ten += 20;
stop3 = true;
} else {
											$('.hangye_wait_ten').text('没有更多新闻了');
										}
									}, "json");;
							}
						} else if (type3 == 'block') {
	...
	offset_ten: offset_ten
									...
									function(data) {
										if (data) {
											$(".brand_wait_ten").before(data);
											$('.brand_wait_ten').text('下拉加载更多');
											offset_ten += 20;
stop4 = true;
										} else {
											$('.brand_wait_ten').text('没有更多新闻了');
										}
									}, "json");;
							}
						}
...
```javascript
<a href="javascript:;" id="scollTopBox">
									<div class="back_to_top  back-top" id="back-top" style="display: none;">
										<img src="img/m_dingb_icon.png" style="width: 100%;height: 100%;">
									</div>
								</a>
```
...
$(document).ready(function() {
				...
				$('.lists li').click(function(event) {
					...
					lists_i = $(this).index();
					$(this).find('.blueLine').css('display', 'block');
					$(this).find('.blueLine').parent().siblings().find('.blueLine').css('display', 'none');
					$('.container .index').eq($(this).index()).css('display', 'block');
					$('.container .index').eq($(this).index()).siblings().css('display', 'none');
					...
					if (lists_i != $(this).index()) {
						$(this).removeClass('active');
						$(this).find('.blueLine').css('display', 'none');
					}
				})
			})
```
...
> <!-- 初始化状态 -->
> 						
> 						<div class="applyProve">
> 							<div class="proveContent">
> 								<div class="proveTitle">奖牌证书认领证明</div>
> 								<div class="proveDesc">请上传奖牌证书认领证明</div>
> 								<div class="proveBtn">
> 									<div class="proveBtn1">
> 										下载模板
> 									</div>
...
> 								</div>
> 							</div>
> 							<div class="proveImg upload_init">
> 								<img src="img/enroll_jiangpai_img.png">
> 								<input type="hidden" name="yingye" id="yingye">
> 								<input type="file" attr="yingye" class="upload_init_file">
> 							</div>
> 						</div>
> ```
...
![1572851644639](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1572851644639.png)
...
li.className = "fkImg";
li.innerHTML = '<img class="showImg" src="'+ e.target.result +'" id="img">'+
'<img src="img/wtfk_svtp_icon.png" class="closeImg" onclick="closeImg(this)" id="img2">';
...
<!-- 二维码窗 -->
		<div class="erCodeTip">
			<div class="erCodeTipMain">
				<a href="javascript:;" class="closeXBtn">
					<img src="images/icon_back_e.png" >
				</a>
				<input type="text" name="captcha" id="captchaHJ" value="点击显示" placeholder="请输入验证码"  />
				<a href="javascript:;" class="erCodeImg" style="display: none;">
					<img class="captchapngHJ" src="http://www.10pinping.com/api/captchas.png.php?rnd=m4528119284739305" title="验证码,看不清楚?请点击刷新字母不区分大小写" alt="" align="absmiddle"id="captchapngHJ" onclick="reloadcaptcha('HJ');" style="cursor:pointer;">
				</a>
...
```javascript
// 点击显示图片验证码
$('#captchaHJ').click(function(){
				$('.erCodeImg').css('display','block')
				$('#captchaHJ').attr('value','')
			})
			function getRand() {
			...
			function showcaptcha(Login) {
				ischeckcaptcha = false;
				if (Dd('captchapng' + Login).style.display == 'none') {
					Dd('captchapng' + Login).style.display = '';
				}
				$('.erCodeImg').show();
...
function sendMessage() {
	...
	// 输入得汉字验证码得汉字
	var then = $('#captchaHJ').val();
	// 判断当汉字验证码不正确得几种情况
if ( then == '点击显示' || then == '' || then.length != 4) {
		alert('验证码不正确')
		$('.codeBtn').click()
		return false;
	} else {
...
$(".codeBtn").text(+curCount1 + "秒再获取");
$('.codeBtn').css('background', '#aaaaaa');
InterValObj1 = window.setInterval(SetRemainTime1, 1000); //启动计时器，1秒执行一次
					$('.closeXBtn').parents('.erCodeTip').hide();
$(".erCodeImg").css('display', 'none');
$('#captchaHJ').val('点击显示');
...
$(".codeBtn").css("pointer-events", "true"); //启用按钮
$(".codeBtn").text("重新发送");
$('.codeBtn').css('background', '#2974B4');
	var erCode = false;
}else {
	curCount1--;
	erCode = false;
...
- **思路：**
- - 将最后结束得时间减去当前的时间，时间差为毫秒，需换算成？h？m？s？ms，判断当小于10时，均需补0，判断当时间均为0时，所有得时间均赋值为0，并清除定时器，最后将换算好得时间填充到其相应位置。

```javascript
// 倒计时
			timeDown('time', '2019-11-5 17:49:00')
			...
			function timeDown(id, endDateStr) {
				...
				if (timeCha>0) {
					var totalSeconds = parseInt(timeCha / 1000);
					var totalHour = parseInt(totalSeconds / 60 / 60);
					var totalMinutes = parseInt((totalSeconds - totalHour * 60 * 60) / 60);
					var seconds = parseInt((totalSeconds - totalHour * 60 * 60) % 60);
...
![1573005170462](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573005170462.png)

![1573005181229](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573005181229.png)

- 描述：当橙色得到最上面时，下面得依次向上走一个

- **html结构**

```javascript
<div class="swiper-rolling">
			<div class="your111"><span><i>36.*.*.154</i>（您）进入了活动</span></div>
			<ul id="jumpLiDiv">
				<li class="firstAct"><span style="background-color: rgba(22, 184, 178, 0.7);"><i style="color: rgba(22, 184, 178, 0.7);">222.*.*.108</i>参与了报名</span></li>
					<li class="secondAct"><span style="background-color: rgba(0, 118, 217, 0.7);"><i style="color: rgba(0, 118, 217, 0.7);">106.*.*.79</i>对品牌进行了投票</span></li>
					<li class="thirdAct"><span style="background-color: rgba(149, 109, 203, 0.7);"><i style="color: rgba(149, 109, 203, 0.7);">106.*.*.39</i>对品牌进行了投票</span></li>
					<li class="fourthAct"><span style="background-color: rgba(22, 184, 178, 0.7);"><i style="color: rgba(22, 184, 178, 0.7);">121.*.*.106</i>申领了证书</span></li>
					<li class=""><span style="background-color: rgba(149, 109, 203, 0.7);"><i style="color: rgba(149, 109, 203, 0.7);">210.*.*.232</i>编辑了品牌介绍</span></li>
					<li class=""><span style="background-color: rgba(22, 184, 178, 0.7);"><i style="color: rgba(22, 184, 178, 0.7);">139.*.*.157</i>编辑了品牌介绍</span></li>
					<li class=""><span style="background-color: rgba(0, 118, 217, 0.7);"><i style="color: rgba(0, 118, 217, 0.7);">182.*.*.138</i>参与了报名</span></li>
					<li class=""><span style="background-color: rgba(149, 109, 203, 0.7);"><i style="color: rgba(149, 109, 203, 0.7);">171.*.*.237</i>对品牌进行了投票</span></li>
					<li class=""><span style="background-color: rgba(22, 184, 178, 0.7);"><i style="color: rgba(22, 184, 178, 0.7);">171.*.*.150</i>编辑了品牌介绍</span></li>
				</ul>
		</div>
```
...
#jumpLiDiv>li:nth-child(n+4) {
	top: 2.22rem;
	opacity: 0;
}

#jumpLiDiv>li:nth-child(3)>span {
	background: rgba(22, 184, 178, 0.7);
}

#jumpLiDiv>li:nth-child(3)>span>i {
	color: #008B86;
}

#jumpLiDiv>li:first-of-type>span {
	background: rgba(0, 118, 217, 0.7);
}

#jumpLiDiv>li:first-of-type>span>i {
	color: #075597;
}

#jumpLiDiv>li:nth-child(2)>span {
	background: rgba(149, 109, 203, 0.7);
}

#jumpLiDiv>li:nth-child(2)>span>i {
	color: #4F2885;
}
...
#jumpLiDiv>li:first-of-type {
	top: 0;
}

#jumpLiDiv>li:nth-child(2) {
	top: 0.74rem;
}

#jumpLiDiv>li:nth-child(3) {
	top: 1.48rem;
}
...
#jumpLiDiv>li.firstAct {
	opacity: 0 !important;
	top: -0.74rem;
	transition: all 1s linear;
}

#jumpLiDiv>li.secondAct {
	transition: all 1s linear;
	top: 0rem;
}

#jumpLiDiv>li.thirdAct {
	transition: all 1s linear;
	top: 0.74rem;
}

#jumpLiDiv>li.fourthAct {
	opacity: 1;
	transition: all 1s linear;
	top: 1.48rem;
}
...
.your111 span{
	background-image: url(../img/ppxq_join_bg.png);
...
- **js实现效果**
- **思路：**
- - 1、将a设置为初始值0，添加一个setInterval定时器，首先需要获取到得是列表得长度，以及前三个背景色。
  - 2.然后给前四个均添加不同得类名,其类名主要是改变top值, 并给其添加了过渡效果，给每个到达前三个位置得重置背景色为前三个得背景色。
  - 3.设置一个setTimeout定时器，【主要是为了将橙色得...进入活动得div从最后面过渡到最前面，通过改变bottom值】将第一个元素插入到id为jumpLiDiv得后面，并给所有得元素移出firstAct-fourthAct得类名。
  - 4.由于最初进入页面只有3个li，因此判断当a=3时，证明已经到最顶部，settimeout定时器将不在执行。
  - 5.当列表得长度为0时，清除setInterval定时器，并移出存放列表id为jumpLiDiv得容器。
  - 6.最后判断当a=0时，将橙色得div放置在最下面，并将a赋值为1

```javascript
	var a = 0;
	activeTextFn()
	
	function activeTextFn() {
		
		// 添加 定时器setInterval()会不停的调用函数
		var timer = setInterval(function(){
			// 获取列表得长度
			var num = $('#jumpLiDiv>li').length;
			// 获取前三个得背景色
var colorSpan1 = $('#jumpLiDiv>li').eq(0).children('span').css('backgroundColor');
var colorSpan2 = $('#jumpLiDiv>li').eq(1).children('span').css('backgroundColor');
			$('#jumpLiDiv>li').eq(2).children('span').css('backgroundColor');
			// 前四个均添加不同得类名,主要改变得是top值, 并给其添加了过渡效果
			$('#jumpLiDiv>li').eq(0).addClass('firstAct');
			// 给其下得span及i标签均给其设置背景色
			$('#jumpLiDiv>li').eq(1).children('span').css('backgroundColor',colorSpan1);
			$('#jumpLiDiv>li').eq(1).addClass('secondAct');
			$('#jumpLiDiv>li').eq(2).children('span').css('backgroundColor',colorSpan2);
			$('#jumpLiDiv>li').eq(2).addClass('thirdAct');
			$('#jumpLiDiv>li').eq(3).children('span').css('backgroundColor',colorSpan3);
			$('#jumpLiDiv>li').eq(3).addClass('fourthAct');
//setTimeout()只执行函数一次
setTimeout(function(){
//append() 方法在被选元素的结尾（仍然在内部）插入指定内容。		$('#jumpLiDiv').append($('#jumpLiDiv>li').eq(0));
				$('#jumpLiDiv>li').removeClass('firstAct secondAct thirdAct fourthAct');
				if(a == 0) {
					a = 1
				}else if(a == 1) {
$('.your111 span').css('bottom','1.18rem');
					a = 2
				}else if(a == 2) {
$('.your111 span').css('bottom','2rem');
					a = 3
				}
			},1960)
			if (num == 0) {
			...
			if(a == 0) {
$('.your111 span').css('bottom','0.5rem');
				a = 1
			}
		},2000)	
	}
...
![1573030033784](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573030033784.png)

- 上面导航栏点击数据跟随切换，下面行业种类点击也要跟着切换
...
.hotJobTop>#tab_nav>li.on {
	background: rgba(2, 97, 177, 0.1);
	border-radius: 0.25rem;
}

.hotJobTop>#tab_nav>li.on>a {
	color: rgba(2, 97, 177, 1);
}


.hotJobTop>#tab_nav>li:nth-child(2) {
	width: 1.5rem;
}

.hotJobTop>#tab_nav>li>a {
	display: block;
	width: auto;
	height: 100%;
	font-size: 0.28rem;
...
- **3.折线中得nav切换**
- - 在导航栏中每个li设置单一属性data-max_ssid固定值，将其获取到得相应得li传给后台，判断是否被选中，更新折线图
...
![1573439817700](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573439817700.png)

- **显示结果为，当滑动到中间显示区域时才显示正常大小，不管左滑还是右滑都会变小**

![1573439887706](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573439887706.png)

![1573439924366](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573439924366.png)

- **具体实现过程**

```javascript
$(document).ready(function() {
				$(".headlines-tip > li").click(function() {
					$(".headlines-tip > li").eq($(this).index()).addClass("active").siblings().removeClass('active');
					$(".headlines-cont > div").hide().eq($(this).index()).show();
				});
				if (window.location.href) {
					var index = 0;
					$(".headlines-tip > li").eq(index).click();
				}
				var headline_obj = {
				...
				var culture_contain = new Swiper('.culture-contain', headline_obj);
				var culture_direction = new Swiper('.culture-direction', headline_obj);
				var culture_thought = new Swiper('.culture-thought', headline_obj);
				var culture_recommend = new Swiper('.culture-recommend', headline_obj);
...
```javascript
<div class="middleView swiper-container">
						<div class="swiper-wrapper">
							<div class="swiper-slide inside">
								<img src="img/home_rmbd_banner_img3.png">
								<div class="item_time" style="opacity: 1;">2019-11-08</div>
								<div class="item_brand" style="opacity: 1;">风电滑环行业</div>
							</div>
							<div class="swiper-slide inside">
								<img src="img/home_rmbd_banner_img3.png">
								<div class="item_time" style="opacity: 1;">2019-11-08</div>
								<div class="item_brand" style="opacity: 1;">缓粘结钢绞线行业</div>
							</div>
							<div class="swiper-slide inside">
								<img src="img/home_rmbd_banner_img3.png">
								<div class="item_time" style="opacity: 1;">2019-11-08</div>
								<div class="item_brand" style="opacity: 1;">树脂砂轮行业</div>
							</div>
							<div class="swiper-slide inside">
								<img src="img/home_rmbd_banner_img3.png">
								<div class="item_time" style="opacity: 1;">2019-11-08</div>
								<div class="item_brand" style="opacity: 1;">单元门口机行业</div>
							</div>
							<div class="swiper-slide inside">
								<img src="img/home_rmbd_banner_img3.png">
								<div class="item_time" style="opacity: 1;">2019-11-08</div>
								<div class="item_brand" style="opacity: 1;">总配线架行业</div>
							</div>
						</div>
						<div class="swiper-pagination middle-swiper-pagination"></div>
					</div>
...
.middleView .swiper-wrapper {
	width: 100%;
}

.middleView .swiper-wrapper .inside {
	width: 6.3rem;
	height: 5.4rem;
	border-radius: 0.1rem;
	position: relative;
...
.swiper-pagination-bullet-active {
	background: rgba(221, 19, 28, 1) !important;
}
```
...
```javascript
var swiperR = new Swiper('.middleView', {
				watchSlidesProgress: true,
				slidesPerView: 'auto',
				// 设置为居中
				centeredSlides: true,
				...
				// window改变尺寸时swiper会重新初始化
				resizeReInit: true,
				// 图片加载完成后重新初始化swiper
				updateOnImageReady: true,
				on: {
					progress: function(preogress) {
for (var i = 0; i < this.slides.length; i++) {
	var slide = this.slides.eq(i);
	var slideProgress = this.slides[i].progress;
							modify = 1;
...
zIndex = 999 - Math.abs(Math.round(10 * slideProgress));
							slide.transform('translateX(' + translate + ') scale(' + scale + ')');
slide.css('zIndex', zIndex)
slide.css('opacity', 1).addClass('shadow').css('background', 'transparent').children('img').css('opacity',
								1).nextAll().css('opacity', 1)
...
![1573442936095](C:\Users\NING MEI\AppData\Roaming\Typora\typora-user-images\1573442936095.png)
...
$('#searching').bind('input propertychange', function(e) {
    ...
    // 判断input框是否为空（若为空清除下拉列表，并隐藏，否则，显示下拉列表请求数据）
	if (txt == '') {
		...
		var html = '';
		var arr = [];// 存放的是从json数据中获取到的符合条件的title名称
$.ajax({
	url: 'vote.json',
	type: 'get',
	dataType: 'json',
	...
	error: function() {						$('.searchHistoryList').css('display', 'none')
	}
})
// 遍历数组，并拼接字符串（下拉 列表li）        
...
// 5s后隐藏边框								setTimeout(function() {									list[i].style.border = 'none'
	}, 5000);
}
})
},
...
- **首先获取到浏览器得路径，先截取到/之后得内容，在将.html截取掉**
- **将最后获取到得内容都放在a标签得类名**
- **判断，当两个相同时，给其列表项增添类名，并显示下面得横线**
...
```javascript
$('.s-swiper').on('mouseenter',function() {
	f_mySwiper.autoplay.stop();
});
$('.s-swiper').on('mouseleave',function() {
	f_mySwiper.autoplay.start();
})
```
